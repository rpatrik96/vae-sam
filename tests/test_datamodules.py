...
def test_ima_data_dims(args):
    ...
    assert obs.shape == torch.Size(
        [module.hparams.batch_size, module.hparams.latent_dim]
    )
    assert labels.shape == torch.Size(
        [module.hparams.batch_size, module.hparams.n_segments]
    )
    assert sources.shape == torch.Size(
        [module.hparams.batch_size, module.hparams.latent_dim]
    )
